...
public class BplusNode<K extends Comparable<K>, V> {
    ...
    public BplusNode(boolean isLeaf) { // 叶子节点或者子节点
        this.isLeaf = isLeaf;
        entries = new ArrayList<Entry<K, V>>();

        if (!isLeaf) { // 不是叶子节点，就有子节点
            children = new ArrayList<BplusNode<K, V>>();
        }
    }

    public BplusNode(boolean isLeaf, boolean isRoot) { // 根节点
        this(isLeaf);
        this.isRoot = isRoot;
    }
    ...
    public void insertOrUpdate(K key, V value, BplusTree<K, V> tree) {
        ...
        if (isLeaf) {
            ...
            if (contains(key) != -1 || entries.size() < tree.getOrder()) {
                insertOrUpdate(key, value);//插入到当前节点的关键字中
                if (tree.getHeight() == 0) {
                    tree.setHeight(1);
                }
                return;
            ...
            // 设置链接
            if (previous != null) {//叶节点的前一个节点
                previous.next = left;
                left.previous = previous;
            }
            if (next != null) {//叶节点的后一个节点
                next.previous = right;
                right.next = next;
            }
            if (previous == null) {
        ...
        if (key.compareTo(entries.get(0).getKey()) < 0) {
            children.get(0).insertOrUpdate(key, value, tree); // 递归调用
